...
const checkUpdatePermissionGuard = (req, res, next) => {
  try {
    const {id: authUserId, role, is_super_admin} = req.user;
    const {id} = req.params;
    if(role === "admin" && is_super_admin) return next();
    if(["admin", "student", "instructor"].includes(role) && authUserId == id) return next();
...
export default checkUpdatePermissionGuard;