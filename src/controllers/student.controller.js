...
import { createUserSchema, updateUserSchema } from "../utils/validators/user.validator.js";
  ...
  constructor() {
    this.create_student = async (req, res) => {
      try {
        ...
        if (checkUser) throw new ClientError("Student already exists", 404);
        value.password = await hashService.hashingPassword(value.password);
        const insertStudent = await UserModel.create({
          ...value,
          is_verified: true,
        });
        const {avatar, otp, otp_time, password, photo_id, ...safeUser} = insertStudent.toJSON();
    ...
    this.get_student = async (req, res) => {
      try {
        ...
        if (!students) throw new ClientError("No students found", 404);
    ...
    this.update_student = async (req, res) => {
      try {
        const {id} = req.params;
        const findStudent = await UserModel.findOne({where: {id, role: "student"}});
        if(!findStudent) throw new ClientError("Student not found", 404);
        const updateData = req.body;
        const {error, value} = updateUserSchema.validate(updateData, {abortEarly: false});
        if(error) throw new ClientError(error.message, 400);
        if(value.password) {
          value.password = await hashService.hashingPassword(value.password);
        };
        await UserModel.update({...value});
        return res.json({message: "Student successfully updated !", status: 200});
      }catch(err) {
        return globalError(err, res);
      };
    };
    this.delete_student = async (req, res) => {
      try {
        const {id} = req.params;
        const findUser = await UserModel.findOne({where: {id, role: "student"}});
        if(!findUser) throw new ClientError("Student not found", 404);
        await UserModel.destroy({where: {id, role: "student"}});
        return res.json({message: "Student successfully deleted !", status: 200});
      }catch(err) {
        return globalError(err, res);
      };
    };
